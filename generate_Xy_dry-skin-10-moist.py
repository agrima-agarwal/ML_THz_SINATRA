...
class patient_analysis:
    def __init__(self,folder,ROI = None):
        try:
            ...
            roi = THzData(folder+'/roi_10min',folder+'/reference.txt',folder+'/baseline.txt')
            con = THzData(folder+'/control_10min',folder+'/reference.txt',folder+'/baseline.txt')
            self.roi = roi
            self.con = con
        except:
            print('no')          
...
black_list = ['S003','S006','S014','S016','S027','S028','S036'] #moi10
D=[]
