...
class patient_analysis:
    def __init__(self,folder,ROI = None):
        try:
            ...
            roi = THzData(folder+'/roi_20min',folder+'/reference.txt',folder+'/baseline.txt')
            con = THzData(folder+'/control_20min',folder+'/reference.txt',folder+'/baseline.txt')
            self.roi = roi
            self.con = con
        except:
            print('no')          
...
black_list = ['S003','S008','S012','S016','S027','S036','S073','S063'] #moi20
D=[]
for j in tqdm(os.listdir('raw files')):
    if j not in black_list:
        Dd = patient_analysis('raw files/'+j)
        D.append(Dd)
print(len(D))
...
np.savez('Xy_dry-skin-20-moist', X=X, y=y)
